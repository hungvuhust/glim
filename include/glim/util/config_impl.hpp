...
// General Eigen IO
template <typename T, int N, int M> struct traits<Eigen::Matrix<T, N, M>> {
...
// Eigen Isometry
template <typename T> struct traits<Eigen::Transform<T, 3, Eigen::Isometry>> {
  using InType  = std::vector<double>;
...
template <> struct traits<std::vector<Eigen::Isometry3d>> {
  using InType  = std::vector<double>;
...
template <typename T>
std::optional<T>
Config::param_nested(const std::vector<std::string> &nested_module_names,
                     const std::string              &param_name) const {
  const auto &json = std::any_cast<const nlohmann::json &>(config);
...
#define DEFINE_CONFIG_IO_SPECIALIZATION(TYPE)                                  \
  ...
  template bool Config::override_param(const std::string &,                    \
