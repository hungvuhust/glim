...
private:
  std::vector<std::uint64_t> collect_neighbor_point_ids(
    const Eigen::Vector3d& point);
  gtsam_points::PointCloudCPU::Ptr collect_submap_points(
...
private:
  ...
  std::vector<glk::PointCloudBuffer::Ptr>
    submap_drawables;  ///< Submap cloud buffers

  std::unordered_map<SubMap::ConstPtr, std::unordered_set<MapCell::Ptr>>
    cell_map;  ///< submap -> cell indices
  std::unordered_map<Eigen::Vector3i, MapCell::Ptr, gtsam_points::Vector3iHash>
    map_cells;  ///< Map cells for fast neighbor query

  // Selection data
  std::vector<std::uint64_t>
    selected_point_ids;  ///< Selected points (submap_id << 32bit) | point_id

  Eigen::Vector3d picked_point;  ///< 3D point picked by a right click
  std::unique_ptr<guik::ModelControl>
    model_control;  ///< Gizmo for manipulating the selection tools
