...
GlobalMappingParams::GlobalMappingParams() {
  ...
  submap_voxel_resolution =
      config.param<double>("global_mapping", "submap_voxel_resolution", 1.0);
  submap_voxel_resolution_max = config.param<double>(
      "global_mapping", "submap_voxel_resolution_max", submap_voxel_resolution);
  submap_voxel_resolution_dmin = config.param<double>(
      "global_mapping", "submap_voxel_resolution_dmin", 5.0);
  submap_voxel_resolution_dmax = config.param<double>(
      "global_mapping", "submap_voxel_resolution_dmax", 20.0);
  ...
  min_implicit_loop_overlap =
      config.param<double>("global_mapping", "min_implicit_loop_overlap", 0.1);
  ...
  isam2_relinearize_thresh =
      config.param<double>("global_mapping", "isam2_relinearize_thresh", 0.1);

  init_pose_damping_scale =
      config.param<double>("global_mapping", "init_pose_damping_scale", 1e10);
}
...
GlobalMapping::GlobalMapping(const GlobalMappingParams &params)
  ...
  if (params.enable_gpu) {
    logger->error(
        "GPU-based factors cannot be used because GLIM is built without GPU "
        "option!!");
  }
#endif
...
void GlobalMapping::insert_submap(const SubMap::Ptr &submap) {
  ...
  if (params.enable_imu) {
    ...
    if (submap->odom_frames.front()->frame_id != FrameID::IMU) {
      logger->warn(
          "odom frames are not estimated in the IMU frame while global mapping "
    ...
    if (current > 0) {
      new_values->insert(
          E(current * 2),
          gtsam::Pose3(
              (submap->T_world_origin * submap->T_origin_endpoint_L).matrix()));
      new_values->insert(
          V(current * 2),
    ...
    new_values->insert(
        E(current * 2 + 1),
        gtsam::Pose3(
            (submap->T_world_origin * submap->T_origin_endpoint_R).matrix()));
    new_values->insert(
        V(current * 2 + 1),
    ...
    if (current != 0) {
      ...
      if (num_integrated < 2) {
        ...
        new_factors->emplace_shared<gtsam::ImuFactor>(
            E(last * 2 + 1), V(last * 2 + 1), E(current * 2), V(current * 2),
            B(last * 2 + 1), imu_integration->integrated_measurements());
      }
    }
  }

  logger->debug("|new_factors|={} |new_values|={}", new_factors->size(),
                new_values->size());
...
void GlobalMapping::insert_submap(int current, const SubMap::Ptr &submap) {
  ...
  const double dist_median =
      gtsam_points::median_distance(submap->frame, max_scan_count);
  const double p = std::max(
      0.0, std::min(1.0, (dist_median - params.submap_voxel_resolution_dmin) /
                             (params.submap_voxel_resolution_dmax -
                              params.submap_voxel_resolution_dmin)));
  const double base_resolution =
...
void GlobalMapping::find_overlapping_submaps(double min_overlap) {
  ...
  double squared_max_implicit_loop_distance =
      params.max_implicit_loop_distance * params.max_implicit_loop_distance;
...
GlobalMapping::create_matching_cost_factors(int current) const {
  ...
  for (int i = 0; i < current; i++) {
    const double squared_dist = (submaps[i]->T_world_origin.translation() -
    ...
    const Eigen::Isometry3d delta =
        submaps[i]->T_world_origin.inverse() * current_submap->T_world_origin;
    const double overlap = gtsam_points::overlap_auto(
  ...
  if (previous_overlap < std::max(0.25, params.min_implicit_loop_overlap)) {
    logger->warn(
        ...
        previous_overlap);
    logger->warn(
        "create a between factor to prevent the submap from being isolated");
    const int          last = current - 1;
    const gtsam::Pose3 init_delta =
        gtsam::Pose3((submaps[last]->T_world_origin.inverse() *
                      submaps[current]->T_world_origin)
...
GlobalMapping::update_isam2(const gtsam::NonlinearFactorGraph &new_factors,
  ...
  try {
    ...
    logger->error(
        "an indeterminant lienar system exception was caught during global map "
        "optimization!!");
    logger->error(e.what());
  ...
  if (indeterminant_nearby_key != 0) {
    ...
    logger->warn("insert a damping factor at {} to prevent corruption",
    ...
    factors.emplace_shared<gtsam_points::LinearDampingFactor>(
        indeterminant_nearby_key, 6, 1e3);
...
void GlobalMapping::save(const std::string &path) {
  ...
  for (const auto &factor : isam2->getFactorsUnsafe()) {
    bool serializable =
        !dynamic_cast<gtsam_points::IntegratedMatchingCostFactor *>(
            factor.get())
#ifdef GTSAM_POINTS_USE_CUDA
  ...
  for (const auto &factor : matching_cost_factors) {
    ...
    if (dynamic_cast<gtsam_points::IntegratedGICPFactor *>(
            factor.second.get())) {
      type = "gicp";
    } else if (dynamic_cast<gtsam_points::IntegratedVGICPFactor *>(
                   factor.second.get())) {
      type = "vgicp";
    }
#ifdef GTSAM_POINTS_USE_CUDA
    else if (dynamic_cast<gtsam_points::IntegratedVGICPFactorGPU *>(
                 factor.second.get())) {
      type = "vgicp_gpu";
    }
#endif
...
bool GlobalMapping::load(const std::string &path) {
  ...
  for (int i = 0; i < num_submaps; i++) {
    ...
    const double base_resolution = params.submap_voxel_resolution +
                                   p * (params.submap_voxel_resolution_max -
                                        params.submap_voxel_resolution);
    ...
    if (params.enable_gpu) {
#ifdef GTSAM_POINTS_USE_CUDA
      subsampled_submaps.back() =
          gtsam_points::PointCloudGPU::clone(*subsampled_submaps.back());
  ...
  if (start_from_frame_id > 0) {
    ...
    auto remove_loc = std::remove_if(
        loaded_graph.begin(), loaded_graph.end(), [](const auto &factor) {
          return dynamic_cast<gtsam::PoseTranslationPrior<gtsam::Pose3> *>(
                     factor.get()) != nullptr;
        });
    logger->info("removed {} prior factors",
                 std::distance(remove_loc, loaded_graph.end()));
    loaded_graph.erase(remove_loc, loaded_graph.end());

    logger->info("removing damping factors");
    remove_loc = std::remove_if(
        loaded_graph.begin(), loaded_graph.end(), [](const auto &factor) {
          return dynamic_cast<gtsam_points::LinearDampingFactor *>(
                     factor.get()) != nullptr;
        });
    logger->info("removed {} prior factors",
                 std::distance(remove_loc, loaded_graph.end()));
    ...
    remove_loc = std::remove_if(
        loaded_graph.begin(), loaded_graph.end(), [](const auto &factor) {
          return dynamic_cast<gtsam::PriorFactor<gtsam::Pose3> *>(
                     factor.get()) != nullptr;
        });
    logger->info("removed {} prior factors",
                 std::distance(remove_loc, loaded_graph.end()));
      ...
      if (params.enable_gpu) {
        ...
        for (const auto &voxelmap : submaps[first]->voxelmaps) {
          graph.emplace_shared<gtsam_points::IntegratedVGICPFactorGPU>(
              X(first), X(second), voxelmap, subsampled_submaps[second], stream,
              buffer);
        }
#else
        logger->warn(
  ...
  if (graph.size() != num_factors_before) {
    logger->warn("removed {} invalid factors",
                 num_factors_before - graph.size());
    needs_recover = true;
  }
  ...
  if (needs_recover) {
    ...
    logger->warn("add {} factors and {} values", recovered.first.size(),
                 recovered.second.size());
...
void GlobalMapping::recover_graph() {
  const auto recovered =
      recover_graph(isam2->getFactorsUnsafe(), isam2->calculateEstimate(), 0);
...
GlobalMapping::recover_graph(const gtsam::NonlinearFactorGraph &graph,
                             const gtsam::Values               &values,
                             int start_from_frame_id) const {
  ...
  for (const auto &factor : graph) {
    ...
    if (factor->keys().size() == 1 && factor->keys()[0] == X(0)) {
      prior_exists |= dynamic_cast<gtsam_points::LinearDampingFactor *>(
                          factor.get()) != nullptr;
    }
  }
  ...
  for (int i = start_from_frame_id; i < submaps.size(); i++) {
    if (!values.exists(X(i))) {
      ...
      new_values.insert(X(i),
                        gtsam::Pose3(submaps[i]->T_world_origin.matrix()));
    }

    if (connectivity_map[X(i)].count(X(i + 1)) == 0 &&
        i != submaps.size() - 1) {
      ...
      new_factors.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
          X(i), X(i + 1), gtsam::Pose3(delta.matrix()), prior_noise6);
    }
    ...
    if (i != 0) {
      if (!values.exists(E(i * 2))) {
        ...
        new_values.insert(E(i * 2), gtsam::Pose3((submap->T_world_origin *
      ...
      if (!values.exists(V(i * 2))) {
        ...
        new_values.insert(
            V(i * 2), (submap->T_world_origin.linear() * v_origin_imuL).eval());
      }
      ...
      if (connectivity_map[B(i * 2)].count(B(i * 2 + 1)) == 0) {
        ...
        new_factors
            .emplace_shared<gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>>(
                B(i * 2), B(i * 2 + 1), gtsam::imuBias::ConstantBias(),
                prior_noise6);
      }
    }
