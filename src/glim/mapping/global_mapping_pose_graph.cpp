...
GlobalMappingPoseGraphParams::GlobalMappingPoseGraphParams() {
  ...
  registration_type =
      config.param<std::string>("global_mapping", "registration_type", "GICP");
  ...
  loop_factor_robust_width =
      config.param<double>("global_mapping", "loop_factor_robust_width", 1.0);

  loop_candidate_buffer_size =
      config.param<int>("global_mapping", "loop_candidate_buffer_size", 100);
  loop_candidate_eval_per_thread =
      config.param<int>("global_mapping", "loop_candidate_eval_per_thread", 2);
...
void GlobalMappingPoseGraph::insert_submap(int                current,
  ...
  if (params.subsample_target > 0) {
    target->subsampled = gtsam_points::random_sampling(
        submap->frame,
        static_cast<double>(params.subsample_target) / submap->frame->size(),
        mt);
  } else {
...
GlobalMappingPoseGraph::create_odometry_factors(int current) const {
  ...
  const int          last = current - 1;
  const gtsam::Pose3 T_last_current =
      gtsam::Pose3((submaps[last]->origin_frame()->T_world_sensor().inverse() *
...
void GlobalMappingPoseGraph::find_loop_candidates(int current) {
  ...
  for (int i = 0; i < submaps.size() - 1; i++) {
    ...
    const double travel_dist =
        submap_targets[current]->travel_dist - submap_targets[i]->travel_dist;
    ...
    const Eigen::Isometry3d T_target_source =
        submaps[i]->T_world_origin.inverse() * submaps[current]->T_world_origin;
...
#pragma omp parallel for num_threads(params.num_threads) schedule(dynamic)
        ...
        tbb::parallel_for(tbb::blocked_range<int>(0, candidates.size(), 2),
