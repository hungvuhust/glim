...
SubMappingPassthroughParams::SubMappingPassthroughParams() {
  ...
  keyframe_update_interval_rot =
    config.param<double>("sub_mapping", "keyframe_update_interval_rot", 0.01);
  keyframe_update_interval_trans =
    config.param<double>("sub_mapping", "keyframe_update_interval_trans", 0.1);

  max_num_keyframes = config.param<int>("sub_mapping", "max_num_keyframes", 50);
  max_num_keyframes =
    max_num_keyframes < 0 ? std::numeric_limits<int>::max() : max_num_keyframes;
...
void SubMappingPassthrough::insert_frame(
  ...
  if (insert_as_keyframe) {
    ...
    auto transformed =
      gtsam_points::transform(odom_frame->frame, odom_frame->T_world_sensor());
...
SubMap::Ptr SubMappingPassthrough::create_submap(bool force_create) const {
  ...
  submap->T_origin_endpoint_L =
    submap->T_world_origin.inverse() * odom_frames.front()->T_world_sensor();
  submap->T_origin_endpoint_R =
    submap->T_world_origin.inverse() * odom_frames.back()->T_world_sensor();
