...
OdometryEstimationCPUParams::OdometryEstimationCPUParams()
  ...
  target_downsampling_rate = config.param<double>("odometry_estimation",
...
gtsam::NonlinearFactorGraph OdometryEstimationCPU::create_factors(
    ...
    for (const auto& voxelmap : target_voxelmaps) {
      auto vgicp_factor =
        gtsam::make_shared<gtsam_points::IntegratedVGICPFactor>(
          gtsam::Pose3(), X(current), voxelmap, frames[current]->frame);
  ...
  T_last_current.linear() =
    Eigen::Quaterniond(T_last_current.linear()).normalized().toRotationMatrix();
  frames[current]->T_world_imu = frames[last]->T_world_imu * T_last_current;
  new_values.insert_or_assign(X(current),
                              gtsam::Pose3(
                                frames[current]->T_world_imu.matrix()));
