...
OdometryEstimationCTParams::OdometryEstimationCTParams() {
  ...
  ivox_min_points_dist =
    config.param<double>("odometry_estimation", "ivox_min_points_dist", 0.05);
  ivox_lru_thresh =
    config.param<int>("odometry_estimation", "ivox_lru_thresh", 30);

  location_consistency_inf_scale =
    config.param<double>("odometry_estimation",
                         "location_consistency_inf_scale",
                         1e-3);
  constant_velocity_inf_scale =
    config.param<double>("odometry_estimation",
                         "constant_velocity_inf_scale",
                         1e-3);
  lm_max_iterations =
...
EstimationFrame::ConstPtr OdometryEstimationCT::insert_frame(
  ...
  covariance_estimation->estimate(raw_frame->points,
                                  raw_frame->neighbors,
  ...
  if (current == 0) {
    ...
    new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
      X(0), gtsam::Pose3(), gtsam::noiseModel::Isotropic::Precision(6, 1e6));
    ...
    if (current >= 2) {
      if (!frames[last] || !frames[last - 1]) {
        ...
        const gtsam::Pose3 delta_pose =
          smoother->calculateEstimate<gtsam::Pose3>(X(last - 1)).inverse() *
    ...
    const auto last_T_world_lidar_begin_ =
      smoother->calculateEstimate<gtsam::Pose3>(X(last));
    const auto last_T_world_lidar_end_ =
      smoother->calculateEstimate<gtsam::Pose3>(Y(last));
    const auto last_T_world_lidar_begin =
      gtsam::Pose3(last_T_world_lidar_begin_.rotation().normalized(),
                   last_T_world_lidar_begin_.translation());
    const auto last_T_world_lidar_end =
      gtsam::Pose3(last_T_world_lidar_end_.rotation().normalized(),
                   last_T_world_lidar_end_.translation());

    // Initial guess
    const double current_time_begin =
      new_frame->stamp + new_frame->frame->times[0];
    const double current_time_end =
      new_frame->stamp + new_frame->frame->times[new_frame->frame->size() - 1];
    const gtsam::Pose3 predicted_T_world_lidar_begin =
      last_T_world_lidar_end *
      gtsam::Pose3::Expmap(last_twist * (current_time_begin - last_time_end));
    ...
    auto                        factor = gtsam::make_shared<
      gtsam_points::IntegratedCT_GICPFactor_<gtsam_points::iVox,
    ...
    auto deskewed_covs =
      covariance_estimation->estimate(deskewed_points, raw_frame->neighbors);
