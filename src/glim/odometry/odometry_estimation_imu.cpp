...
OdometryEstimationIMUParams::OdometryEstimationIMUParams() {
  ...
  initialization_mode = config.param<std::string>("odometry_estimation",
                                                  "initialization_mode",
                                                  "LOOSE");
  const auto init_T_world_imu =
    config.param<Eigen::Isometry3d>("odometry_estimation", "init_T_world_imu");
  const auto init_v_world_imu =
    config.param<Eigen::Vector3d>("odometry_estimation", "init_v_world_imu");
  ...
  this->init_pose_damping_scale =
    config.param<double>("odometry_estimation",
  ...
  isam2_relinearize_thresh = config.param<double>("odometry_estimation",
                                                  "isam2_relinearize_thresh",
                                                  0.1);

  save_imu_rate_trajectory = config.param<bool>("odometry_estimation",
...
OdometryEstimationIMU::OdometryEstimationIMU(
  ...
  if (!params->estimate_init_state || params->initialization_mode == "NAIVE") {
    auto init_estimation =
      new NaiveInitialStateEstimation(params->T_lidar_imu, params->imu_bias);
    ...
    auto init_estimation =
      new LooseInitialStateEstimation(params->T_lidar_imu, params->imu_bias);
    this->init_estimation.reset(init_estimation);
  } else {
...
#ifdef GTSAM_USE_TBB
  tbb_task_arena =
    std::make_shared<tbb::task_arena>(params->num_smoother_update_threads);
#endif
}
...
EstimationFrame::ConstPtr OdometryEstimationIMU::insert_frame(
  ...
  if (raw_frame->size()) {
    logger->trace("insert_frame points={} times={} ~ {}",
                  raw_frame->size(),
  ...
  if (frames.empty()) {
    ...
    covariance_estimation->estimate(points_imu,
                                    raw_frame->neighbors,
                                    normals,
                                    covs);
  ...
  const auto   last_T_world_imu_ =
    smoother->calculateEstimate<gtsam::Pose3>(X(last));
  const auto last_T_world_imu =
    gtsam::Pose3(last_T_world_imu_.rotation().normalized(),
                 last_T_world_imu_.translation());
  ...
  // Overwrite the predicted state with the last states if no IMU data is
  // available
  if (num_imu_integrated < 2 && last > 1) {
    const Eigen::Isometry3d T_delta =
      frames[last - 1]->T_lidar_imu.inverse() * frames[last]->T_lidar_imu;
    predicted_T_world_imu =
      gtsam::Pose3((frames[last]->T_world_imu * T_delta).matrix());
  ...
  if (num_imu_integrated >= 2) {
    ...
    logger->warn(
      ...
      "(odometry_estimation)");
    logger->warn("t_last={:.6f} t_current={:.6f} num_imu={}",
                 last_stamp,
                 raw_frame->stamp,
                 num_imu_integrated);
  ...
  imu_integration->integrate_imu(raw_frame->stamp,
  ...
  auto deskewed = deskewing->deskew(T_imu_lidar,
                                    pred_imu_times,
                                    pred_imu_poses,
                                    raw_frame->stamp,
                                    raw_frame->times,
  ...
  covariance_estimation->estimate(deskewed,
                                  raw_frame->neighbors,
                                  deskewed_normals,
                                  deskewed_covs);
  ...
  std::vector<EstimationFrame::ConstPtr> active_frames(frames.begin() +
                                                         marginalized_cursor,
                                                       frames.end());
...
std::vector<EstimationFrame::ConstPtr> OdometryEstimationIMU::
...
void OdometryEstimationIMU::update_frames(
  ...
  logger->trace("update frames current={} marginalized_cursor={}",
                current,
                marginalized_cursor);
...
void OdometryEstimationIMU::update_smoother(int count) {
  ...
  update_smoother(gtsam::NonlinearFactorGraph(),
                  gtsam::Values(),
