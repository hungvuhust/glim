...
void DataValidator::points_callback(
  ...
  if (last_points_stamp >= 0.0) {
    if (stamp < last_points_stamp) {
      ...
      spdlog::warn(
          "A large time gap between consecutive point clouds!! (last={:.6f} "
          "current={:.6f} diff={:.6f})",
  ...
  if (last_imu_stamp >= 0.0) {
    if (std::abs(last_imu_stamp - stamp) > 1.0) {
      spdlog::warn(
          "Too large time gap between points and IMU timestamps!! (IMU={:.6f} "
          "points={:.6f})",
          last_imu_stamp, stamp);
    }
  }
  ...
  if (std::any_of(raw_points->times.begin(), raw_points->times.end(),
    ...
    spdlog::warn(
        "A negative per-point timestamp found!! (t={:.6f})",
        *std::min_element(raw_points->times.begin(), raw_points->times.end()));
  }

  if (std::any_of(raw_points->times.begin(), raw_points->times.end(),
                  [](double t) { return t > 1.0; })) {
    spdlog::warn(
        "A large per-point timestamp found!! (t={:.6f})",
        *std::max_element(raw_points->times.begin(), raw_points->times.end()));
  }
