...
#define PRINT_IF_DEFINED(logger, name)                                         \
  logger->debug(#name " = {}", IS_DEFINED_##name)

namespace glim {

void print_system_info(std::shared_ptr<spdlog::logger> logger) {
  logger->debug("*** GTSAM ***");
  logger->debug("GTSAM_VERSION={}.{}.{} ({})", GTSAM_VERSION_MAJOR,
                GTSAM_VERSION_MINOR, GTSAM_VERSION_PATCH, GTSAM_VERSION_STRING);
  ...
  logger->debug("GTSAM_EIGEN_VERSION={}.{}", GTSAM_EIGEN_VERSION_WORLD,
                GTSAM_EIGEN_VERSION_MAJOR);
#else
  logger->debug("GTSAM_EIGEN_VERSION is not defined");
#endif

  logger->debug("*** GTSAM_POINTS ***");
  logger->debug("GTSAM_POINTS_VERSION={}.{}.{} ({})",
                GTSAM_POINTS_VERSION_MAJOR, GTSAM_POINTS_VERSION_MINOR,
                GTSAM_POINTS_VERSION_PATCH, GTSAM_POINTS_VERSION_STRING);
  ...
  logger->debug("*** CUDA ***");
  logger->debug(
      "GTSAM_POINTS_CUDA_VERSION={}.{}.{}", GTSAM_POINTS_CUDA_VERSION_MAJOR,
      GTSAM_POINTS_CUDA_VERSION_MINOR, GTSAM_POINTS_CUDA_VERSION_PATCH);
