...
void serializeToBinaryFile(const gtsam::NonlinearFactorGraph &graph,
                           const std::string &path, bool only_serializable) {
  try {
  ...
  spdlog::warn(
      "retyring to serialize factor graph with only serializable factor types");
...
void serializeToBinaryFile(const gtsam::Values &values, const std::string &path,
