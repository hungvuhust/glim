...
PointsSelector::PointsSelector(std::shared_ptr<spdlog::logger> logger)
    : picked_point(0, 0, 0), logger(logger) {
...
void PointsSelector::set_submaps(
    const std::vector<glim::SubMap::Ptr> &submaps) {
  this->submaps = submaps;
  this->submap_drawables.resize(submaps.size());
  std::transform(submaps.begin(), submaps.end(), this->submap_drawables.begin(),
                 [](const SubMap::Ptr &submap) {
                   return guik::viewer()->update_points(
                       "submap_" + std::to_string(submap->id),
                       submap->frame->points, submap->frame->size(),
                       guik::Rainbow(submap->T_world_origin));
                 });
...
/// @return Point ids within the search window. Point ID = (submap_id << 32bit)
/// | point_id
std::vector<std::uint64_t>
PointsSelector::collect_neighbor_point_ids(const Eigen::Vector3d &point) {
  // Center of the voxel grid search window
  const Eigen::Vector3i center =
      (point.array() / map_cell_resolution).floor().cast<int>();
  ...
  for (const auto &cell : selected_cells) {
    selected_point_ids.insert(selected_point_ids.end(), cell->point_ids.begin(),
                              cell->point_ids.end());
  }
  logger->info("|selected_cells|={} |selected_points|={}",
               selected_cells.size(), selected_point_ids.size());
...
gtsam_points::PointCloudCPU::Ptr PointsSelector::collect_submap_points(
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 32)
  for (int i = 0; i < point_ids.size(); i++) {
    ...
    points->points_storage[i] =
        submap->T_world_origin * submap->frame->points[point_id];
    if (has_normals) {
      points->normals_storage[i] =
          submap->T_world_origin.matrix() * submap->frame->normals[point_id];
    }
    if (has_covs) {
      points->covs_storage[i] = submap->T_world_origin.matrix() *
                                submap->frame->covs[point_id] *
                                submap->T_world_origin.matrix().transpose();
    }
    // if (has_intensities) {
    //   points->intensities_storage[i] =
    //   (submaps[submap_id]->frame->intensities[point_id]);
    // }
  }
...
void PointsSelector::update_cells() {
  ...
  for (const auto &submap : submaps) {
    ...
    for (int i = 0; i < submap->frame->size(); i++) {
      const Eigen::Vector4d pt =
          submap->T_world_origin * submap->frame->points[i];
      const Eigen::Vector3i coord =
          gtsam_points::fast_floor(pt * inv_resolution).head<3>();
  ...
  for (const auto &cell : map_cells) {
    ...
    const Eigen::Vector3d center =
        coord.cast<double>() * map_cell_resolution +
        Eigen::Vector3d::Constant(map_cell_resolution * 0.5);
    container->push_back(glk::Primitives::wire_cube(),
                         guik::FlatGreen()
                             .translate(center)
                             .scale(map_cell_resolution)
                             .set_alpha(0.5));
  }

  guik::viewer()->update_drawable("cells", container,
                                  guik::FlatGreen().set_alpha(0.5));
...
void PointsSelector::draw_ui() {
  ...
  if (show_preferences_window &&
      ImGui::Begin("Preferenecs", &show_preferences_window,
                   ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::DragInt("Num threads", &num_threads, 1, 1,
                   std::thread::hardware_concurrency()) ||
        show_note("Number of threads for processing");
    ...
    if (ImGui::Button("Rectangle") ||
        show_note("Set point shape to rectangle")) {
  ...
  if (ImGui::Begin("Editor", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::BeginMenu("Map cells config")) {
      ...
      ImGui::DragFloat("map cell resolution", &map_cell_resolution, 0.1f, 0.1f,
                       10.0f) ||
          show_note("Map cell resolution [m]");
      ImGui::DragInt("selection window", &cell_selection_window, 1, 1, 10) ||
          show_note("Neighbor cell selection window");
    ...
    if (ImGui::Button("Select points") ||
        ...
        show_note("Select points within the selection tool")) {
      select_points_tool();
    }
    ImGui::SameLine();
    ...
    ImGui::Separator();
    if (ImGui::Button("Remove selected points") ||
        (ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_R)) ||
        show_note("Remove selected points from the submap")) {
      remove_selected_points();
    }
    ImGui::SameLine();
    ImGui::TextDisabled("(Ctrl+R)");

    if (ImGui::Button("Unselect points") ||
        show_note("Clear selected points")) {
  ...
  if (pick) {
    ...
    viewer->update_points(
        "selected_cells", selected_points->points, selected_points->size(),
        guik::FlatOrange().set_alpha(0.8).set_point_scale(2.0f));
  ...
  if (ImGui::BeginPopup("context_menu", ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    ImGui::Text("Point: (%.1f, %.1f, %.1f)", this->picked_point.x(),
                this->picked_point.y(), this->picked_point.z());
    ImGui::Text("Selected points: %ld", selected_point_ids.size());
    ImGui::Separator();
    ...
    const bool gizmo_selection_clicked =
    ...
    if (radius_menu_open) {
      ...
      if (ImGui::BeginMenu("Outlier selection")) {
        ImGui::DragFloat("Radius offset", &outlier_radius_offset, 0.01f, 0.01f,
                         100.0f);
        ...
        ImGui::DragFloat("Stddev threshold", &outlier_stddev_thresh, 0.01f,
                         0.01f, 10.0f);
    ...
    if (segmentation_menu_open) {
      ...
      show_note(
          "Segmentation method\n"
          "- MinCut: Graph cut based segmentation for extracting an object in "
          "a radius\n"
          "- RegionGrowing: Region growing based segmentation for extracting a "
          "connected region");
      ...
      if (segmentation_method == 0) {
        ...
        ImGui::DragFloat("Distance sigma", &distance_sigma, 0.01f, 0.01f,
                         10.0f);
        ...
        ImGui::DragFloat("Foreground radius", &foreground_mask_radius, 0.01f,
                         0.01f, 100.0f);
        show_note("Points within this radius are marked as foreground");
        ImGui::DragFloat("Background radius", &background_mask_radius, 0.01f,
                         0.01f, 100.0f);
        ...
        float angle_threshold =
            region_growing_params.angle_threshold * 180.0 / M_PI;
        float dilation_radius = region_growing_params.dilation_radius;

        ImGui::DragFloat("Distance threshold", &distance_threshold, 0.01f,
                         0.01f, 1.0f);
        show_note("Points connection distance threshold [m]");
        ImGui::DragFloat("Angle threshold", &angle_threshold, 0.01f, 0.01f,
                         180.0f);
        show_note("Points connection angle threshold [deg]");
        ImGui::DragFloat("Dilation radius", &dilation_radius, 0.01f, 0.01f,
                         100.0f);
        show_note(
            "After performing region growing, the extracted region will be "
            "dilated by this radius to fill halls");
  ...
  if (show_picked_point) {
    viewer->update_sphere(
        "picked_point",
        guik::FlatRed().translate(this->picked_point).scale(0.2));
  } else {
    viewer->remove_drawable("picked_point");
  }

  if (show_selection_radius) {
    viewer->update_wire_sphere("selection_radius",
                               guik::FlatRed()
                                   .translate(this->picked_point)
                                   .scale(select_radius)
                                   .set_alpha(0.5));
  } else {
  ...
  if (show_segmentation_radius && segmentation_method == 0) {
    viewer->update_wire_sphere("mincut_foreground_radius",
                               guik::FlatRed()
                                   .translate(this->picked_point)
                                   .scale(min_cut_params.foreground_mask_radius)
                                   .set_alpha(0.5));
    viewer->update_wire_sphere("mincut_background_radius",
                               guik::FlatBlue()
                                   .translate(this->picked_point)
                                   .scale(min_cut_params.background_mask_radius)
                                   .set_alpha(0.5));
  ...
  if (show_gizmo) {
    ...
    case 0: // Box
      viewer->update_cube(
          "selection_tool",
          guik::FlatOrange(model_control->model_matrix()).set_alpha(0.5));
      break;
    case 1: // Sphere
      viewer->update_sphere(
          "selection_tool",
...
/// @note  Parallelism is limited to the number of submaps in the current
/// implementation
void PointsSelector::remove_selected_points() {
  std::sort(selected_point_ids.begin(), selected_point_ids.end());

  std::vector<
      std::tuple<SubMap::Ptr, const std::uint64_t *, const std::uint64_t *>>
      updated_submaps; // (submap, begin, end)
  ...
  while (point_id_itr != selected_point_ids.end()) {
    ...
    const auto end   = std::find_if(point_id_itr, selected_point_ids.end(),
                                    [=](std::uint64_t id) {
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic)
  for (int i = 0; i < updated_submaps.size(); i++) {
    ...
    const auto remove_loc =
        std::remove_if(point_indices.begin(), point_indices.end(),
                       [](int id) { return id == -1; });
  ...
  std::vector<std::pair<MapCell *, std::vector<int>>> cells_submaps_vec(
      cells_submaps.begin(), cells_submaps.end());
...
#pragma omp critical
  ...
  for (const auto &submap_ : updated_submaps) {
    ...
    submap_drawables[submap->id] = guik::viewer()->update_points(
        "submap_" + std::to_string(submap->id), submap->frame->points,
        submap->frame->size(), guik::Rainbow(submap->T_world_origin));
  }
...
void PointsSelector::select_points_tool() {
  ...
  const Eigen::Matrix4d inv_model_matrix =
      model_control->model_matrix().cast<double>().inverse();
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic)
  for (int submap_idx = 0; submap_idx < submaps.size(); submap_idx++) {
    ...
    const Eigen::Matrix4d T_local_world =
        inv_model_matrix * submap->T_world_origin.matrix();

    std::vector<std::uint64_t>
        local_selected_point_ids; // Thread-local selected point ids
    ...
    for (int i = 0; i < submap->frame->size(); i++) {
      ...
      // Cube
      if (selected_tool == 0) {
        if ((pt.array() > Eigen::Array4d(-0.5, -0.5, -0.5, 0.0)).all() &&
            (pt.array() < Eigen::Array4d(0.5, 0.5, 0.5, 2.0)).all()) {
          local_selected_point_ids.emplace_back(
              (static_cast<std::uint64_t>(submap->id) << 32) | i);
        }
      }
      // Sphere
      else {
...
#pragma omp critical
    {
      selected_point_ids.insert(selected_point_ids.end(),
                                local_selected_point_ids.begin(),
                                local_selected_point_ids.end());
    }
  }
  ...
  guik::viewer()->update_points(
      "selected", selected_points->points, selected_points->size(),
      guik::FlatOrange().set_alpha(0.8).set_point_scale(2.0f));
  show_gizmo = false;
}
...
void PointsSelector::select_points_radius() {
  ...
  for (int i = 0; i < neighbor_points->size(); i++) {
    const double dist_sq =
        (neighbor_points->points[i] - picked_point.homogeneous()).squaredNorm();
    if (dist_sq < sq_thresh) {
      selected_point_ids.emplace_back(neighbor_point_ids[i]);
    }
  }

  logger->info("Select points in radius: {} points selected",
               selected_point_ids.size());

  auto filtered = collect_submap_points(selected_point_ids);
  guik::viewer()->update_points(
      "selected", filtered->points, filtered->size(),
      guik::FlatOrange().set_alpha(0.8).set_point_scale(2.0f));
}
...
void PointsSelector::select_outlier_points_radius() {
  ...
  const double offset_sq_thresh =
      std::pow(select_radius + outlier_radius_offset, 2);
  for (int i = 0; i < neighbor_points->size(); i++) {
    const double dist_sq =
        (neighbor_points->points[i] - picked_point.homogeneous()).squaredNorm();
    if (dist_sq < offset_sq_thresh) {
      point_ids_in_radius.emplace_back(
          dist_sq < sq_thresh ? neighbor_point_ids[i] : INLIER);
  ...
  auto tree = std::make_shared<gtsam_points::KdTree2<gtsam_points::PointCloud>>(
      points_in_radius);
  ...
  for (int i = 0; i < points_in_radius->size(); i++) {
    ...
    tree->knn_search(points_in_radius->points[i].data(), k, k_indices.data(),
                     k_sq_dists.data());
    std::copy(k_indices.begin(), k_indices.end(), neighbors.begin() + i * k);
  }

  auto inliers = gtsam_points::find_inlier_points(points_in_radius, neighbors,
                                                  k, outlier_stddev_thresh);
  for (int idx : inliers) {
    point_ids_in_radius[idx] = INLIER;
  }

  auto remove_loc =
      std::remove_if(point_ids_in_radius.begin(), point_ids_in_radius.end(),
                     [](std::uint64_t id) { return id == INLIER; });
...
void PointsSelector::select_points_segmentation() {
  ...
  if (segmentation_method == 0) {
    ...
    const double               sq_dist_thresh =
        std::pow(min_cut_params.background_mask_radius + 1.0, 2);
    for (int i = 0; i < neighbor_point_ids.size(); i++) {
      const double sq_dist =
          (neighbor_points->points[i] - picked_point.homogeneous())
              .squaredNorm();
    ...
    // Collect points within the background mask radius and estimate their
    // normals
    auto filtered = collect_submap_points(filtered_ids);
    filtered->add_normals(gtsam_points::estimate_normals(
        filtered->points, filtered->size(), 20, num_threads));
    auto filtered_tree =
    ...
    const auto result          = gtsam_points::min_cut(
                 *filtered, *filtered_tree, picked_point.homogeneous(), min_cut_params);
  ...
  // Region growing
    ...
    auto rg                           = gtsam_points::region_growing_init(
                                  *filtered, *kdtree, picked_point.homogeneous(), region_growing_params);
    gtsam_points::region_growing_update(rg, *filtered, *kdtree,
                                        region_growing_params);

    for (int i = 0; i < rg.cluster_indices.size(); i++) {
      selected_point_ids.emplace_back(
          neighbor_point_ids[rg.cluster_indices[i]]);
    }
  }
  ...
  guik::viewer()->update_points(
      "selected", selected_points->points, selected_points->size(),
      guik::FlatOrange().set_alpha(0.8).set_point_scale(2.0f));
  logger->info("Select points by segmentation: {} points selected",
