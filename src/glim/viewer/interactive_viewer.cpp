...
InteractiveViewer::InteractiveViewer()
  ...
  point_size_metric =
      config.param("interactive_viewer", "point_size_metric", false);
  point_shape_circle =
      config.param("interactive_viewer", "point_shape_circle", true);

  z_range = config
                .param("interactive_viewer", "default_z_range",
  ...
  GlobalMappingCallbacks::on_smoother_update_result.add(std::bind(
      &InteractiveViewer::globalmap_on_smoother_update_result, this, _1, _2));
...
void InteractiveViewer::drawable_selection() {
  ...
  if (ImGui::Combo("ColorMode", &color_mode, color_modes.data(),
                   color_modes.size())) {
    update_viewer();
  }
  show_note(
      "Color mode for rendering submaps.\n- RAINBOW=Altitute encoding color\n- "
  ...
  if (ImGui::BeginMenu("Display settings")) {
    ...
    if (ImGui::Checkbox("Cumulative rendering", &enable_partial_rendering)) {
      ...
      for (int i = 0;; i++) {
        ...
        auto cb = std::dynamic_pointer_cast<const glk::PointCloudBuffer>(
            found.second);
        auto cloud_buffer =
  ...
  if (ImGui::Button("Find overlapping submaps") ||
      show_note("Find overlapping submaps and create matching cost factors "
                "between them.")) {
  ...
  if (ImGui::Button("Recover graph") ||
      show_note("Detect and fix corrupted graph.")) {
  ...
  if (submaps.size() && needs_session_merge) {
    if (ImGui::Button("Merge sessions") ||
        show_note(
            "Merge the lastly loaded session with the previous session.")) {
      ...
      if (submaps.empty()) {
        ...
        const auto source_begin =
            std::find_if(submaps.begin(), submaps.end(),
                         [=](const SubMap::ConstPtr &submap) {
                           return submap->session_id == source_session_id;
                         });

        const std::vector<SubMap::ConstPtr> target_submaps(submaps.begin(),
                                                           source_begin);
        const std::vector<SubMap::ConstPtr> source_submaps(source_begin,
                                                           submaps.end());

        logger->info(
            "|submaps|={} |targets|={} |sources|={} source_session_id={}",
            submaps.size(), target_submaps.size(), source_submaps.size(),
            source_session_id);
  ...
  if (ImGui::BeginPopupContextVoid("context menu")) {
    ...
    if (type == PickType::FRAME) {
      ...
      ImGui::TextUnformatted(
          ("Submap ID : " + std::to_string(frame_id)).c_str());
      if (ImGui::MenuItem("Loop begin", nullptr,
                          manual_loop_close_modal->is_target_set())) {
        manual_loop_close_modal->set_target(
            X(frame_id), submaps[frame_id]->frame, submap_poses[frame_id]);
      }
      if (ImGui::MenuItem("Loop end", nullptr,
                          manual_loop_close_modal->is_source_set())) {
...
void InteractiveViewer::update_viewer() {
  ...
  for (int i = 0; i < submaps.size(); i++) {
    ...
    if (drawable.first) {
      ...
      const Eigen::Vector4f color = glk::colormap_categoricalf(
          glk::COLORMAP::TURBO, submap->session_id, 6);
      ...
      if (submap->frame->has_intensities()) {
        cloud_buffer->add_intensity(
            glk::COLORMAP::TURBO, submap->frame->intensities,
            submap->frame->size(),
            1.0 / *std::max_element(submap->frame->intensities,
    ...
    viewer->update_drawable(
        ...
        guik::FlatColor(1.0f, 0.0f, 0.0f, 0.5f,
                        submap_pose *
                            Eigen::UniformScaling<float>(sphere_scale))
            .add("info_values", info)
            .make_transparent());
  }
  ...
  const auto get_position = [this](const gtsam::Key key) -> Eigen::Vector3d {
    ...
    switch (symbol.chr()) {
    case 'x': return submaps[symbol.index()]->T_world_origin.translation();
    case 'e': {
      const int right     = symbol.index() % 2;
      const int submap_id = (symbol.index() - right) / 2;

      const auto &submap = submaps[submap_id];
      const auto &T_origin_endpoint =
          right ? submap->T_origin_endpoint_R : submap->T_origin_endpoint_L;
  ...
  viewer->update_drawable(
      "factors", std::make_shared<glk::ThinLines>(factor_lines, factor_colors),
...
void InteractiveViewer::globalmap_on_update_submaps(
  ...
  std::transform(
      updated_submaps.begin(), updated_submaps.end(), poses.begin(),
      [](const SubMap::ConstPtr &submap) { return submap->T_world_origin; });
...
void InteractiveViewer::globalmap_on_smoother_update(
  ...
  // Explicitly move the poses of merged submaps to the coordinate system of the
  // origin of the first session
  for (const auto &factor : factors) {
    ...
    logger->info(
        "Moving newly added submaps to the coordinate system of the origin of "
        "the first session");
    ...
    const gtsam::Pose3 T_worldt_target(
        submaps[gtsam::Symbol(target).index()]->T_world_origin.matrix());
    const gtsam::Pose3 T_worlds_source(
        submaps[gtsam::Symbol(source).index()]->T_world_origin.matrix());
    const gtsam::Pose3 T_worldt_worlds =
        T_worldt_target * T_target_source * T_worlds_source.inverse();
    ...
    for (const auto &value : new_values) {
      ...
      new_values.insert_or_assign(
          value.key, T_worldt_worlds * value.value.cast<gtsam::Pose3>());
      moved_count++;
    }
  ...
  for (const auto &factor : new_factors) {
    ...
    if (dynamic_cast<gtsam_points::IntegratedMatchingCostFactor *>(
            factor.get())) {
      inserted_factors.push_back(std::make_tuple(
