...
StandardViewer::StandardViewer() : logger(create_module_logger("viewer")) {
  ...
  z_range_mode = 0;
  z_range      = config
                .param("standard_viewer", "default_z_range",
...
Eigen::Isometry3f
StandardViewer::resolve_pose(const EstimationFrame::ConstPtr &frame) {
  switch (frame->frame_id) {
  case FrameID::WORLD: return Eigen::Isometry3f::Identity();

  default: return trajectory->odom2world(frame->T_world_sensor()).cast<float>();
  }
}
...
void StandardViewer::set_callbacks() {
      ...
      [this](const PreprocessedFrame::ConstPtr &frame,
        ...
        invoke([this, frame, T_odom_lidar] {
          ...
          viewer->update_drawable("initialization_frame_" + guik::anon(),
                                  cloud_buffer, guik::FlatBlue(*T_odom_lidar));
          ...
          case 2: // NORMAL
            if (new_frame->frame->normals) {
              std::vector<Eigen::Vector4d> normals(
                  ...
                  new_frame->frame->normals + new_frame->frame->size());
      ...
      [this](const std::vector<EstimationFrame::ConstPtr> &keyframes) {
        invoke([this, keyframes] {
          ...
          for (const auto &keyframe : keyframes) {
            ...
            viewer->update_drawable(
                "odometry_keyframe_coord_" + std::to_string(keyframe->id),
                glk::Primitives::coordinate_system(), guik::VertexColor(pose));
          }
        });
      });
      ...
      [this](gtsam_points::IncrementalFixedLagSmootherExtWithFallback &smoother,
        ...
        for (const auto &factor : new_factors) {
          ...
          if (factor->keys().size() == 1) {
            ...
            if (matching_factor) {
              const auto l = [this, idx0](const gtsam::NonlinearFactor *factor)
                ...
                const Eigen::Vector3d pt1 =
                    static_cast<
                        const gtsam_points::IntegratedMatchingCostFactor *>(
                        factor)
            ...
            const auto gpu_factor =
                dynamic_cast<gtsam_points::IntegratedVGICPFactorGPU *>(
                    factor.get());
            if (gpu_factor) {
              const auto l = [this, idx0](const gtsam::NonlinearFactor *factor)
                  -> std::optional<FactorLine> {
            ...
            const auto l = [this, idx0, idx1](const gtsam::NonlinearFactor *)
              ...
              return std::make_tuple(
                  found0->second.translation(), found1->second.translation(),
                  Eigen::Vector4f(0.0f, 1.0f, 0.0f, factors_alpha),
                  Eigen::Vector4f(0.0f, 1.0f, 0.0f, factors_alpha));
            };
        ...
        invoke([this, new_factor_lines] {
          auto remove_loc = std::remove_if(
              odometry_factor_lines.begin(), odometry_factor_lines.end(),
              [](const auto &factor) { return factor.first.expired(); });
          ...
          viewer->update_drawable(
              "odometry_factors",
              std::make_shared<glk::ThinLines>(line_vertices, line_colors),
  ...
  OdometryEstimationCallbacks::on_marginalized_frames.add(
      [this](const std::vector<EstimationFrame::ConstPtr> &frames) {
        ...
        std::transform(
            frames.begin(), frames.end(), marginalized_ids.begin(),
            [](const EstimationFrame::ConstPtr &frame) { return frame->id; });
  ...
  GlobalMappingCallbacks::on_insert_submap.add(
      [this](const SubMap::ConstPtr &submap) {
        ...
        // To safely pass it to the UI thread (pass-by-value of Eigen classes
        // can crash) wrap it in a shared_ptr
  ...
  GlobalMappingCallbacks::on_update_submaps.add(
      [this](const std::vector<SubMap::Ptr> &submaps) {
        ...
        invoke([this, latest_submap, submap_ids, submap_poses, mem_stats] {
          ...
          submap_memstats.insert(submap_memstats.end(), mem_stats.begin(),
                                 mem_stats.end());
      ...
      [this](gtsam_points::ISAM2Ext             &isam2,
        ...
        if (show_memory_stats) {
          ...
          for (int i = global_factor_stats_count;
               i < isam2.getFactorsUnsafe().size(); i++) {
...
void StandardViewer::drawable_selection() {
  ...
  update_z_range |= ImGui::Combo("z_range_mode", &z_range_mode,
                                 z_range_modes.data(), z_range_modes.size());
  update_z_range |= ImGui::DragFloatRange2("z_min", &z_range[0], &z_range[1],
                                           0.1f, -10000.0f, 10000.0f);
  if (update_z_range) {
    Eigen::Vector2f z = z_range;
    if (z_range_mode == 0) {
      z += auto_z_range;
  ...
  if (show_mapping_tools) {
    ImGui::Begin("mapping tools", &show_mapping_tools,
                 ImGuiWindowFlags_AlwaysAutoResize);
  ...
  if (show_memory_stats) {
    ImGui::Begin("memory stats", &show_memory_stats,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    if (ImGui::BeginTable("Global mapping memory usage", 3,
