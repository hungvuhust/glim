...
StandardViewer::StandardViewer() : logger(create_module_logger("viewer")) {
  ...
  z_range =
    config
      .param("standard_viewer", "default_z_range", Eigen::Vector2d(-2.0, 4.0))
      .cast<float>();
  ...
  point_size_metric =
    config.param("standard_viewer", "point_size_metric", false);
  point_shape_circle =
    config.param("standard_viewer", "point_shape_circle", true);
  ...
  while (!viewer_started && std::chrono::high_resolution_clock::now() - t1 <
...
void StandardViewer::set_callbacks() {
  ...
  OdometryEstimationCallbacks::on_new_frame.add(
    [this](const EstimationFrame::ConstPtr& new_frame) {
      invoke([this, new_frame] {
        auto viewer = guik::LightViewer::instance();
        auto cloud_buffer =
          std::make_shared<glk::PointCloudBuffer>(new_frame->frame->points,
        ...
        last_median_distance =
          gtsam_points::median_distance(new_frame->frame, 256);
        ...
        switch (current_color_mode) {
          ...
          case 1:  // INTENSITY
            if (new_frame->raw_frame &&
              ...
              const double max_intensity =
                *std::max_element(new_frame->raw_frame->intensities.begin(),
              ...
              std::vector<float> intensities(new_frame->frame->intensities,
          ...
          case 2:  // NORMAL
            if (new_frame->frame->normals) {
              std::vector<Eigen::Vector4d> normals(new_frame->frame->normals,
        ...
        viewer->update_drawable("current_frame",
                                cloud_buffer,
                                shader_setting.add("point_scale", 2.0f));
        viewer->update_drawable("current_coord",
                                glk::Primitives::coordinate_system(),
                                guik::VertexColor(
                                  pose * Eigen::UniformScaling<float>(1.5f)));
        viewer->update_drawable("frame_" + std::to_string(new_frame->id),
                                cloud_buffer,
                                shader_setting_rainbow);
      });
    });
    ...
    [this](const std::vector<EstimationFrame::ConstPtr>& frames) {
      invoke([this, frames] {
        ...
        for (const auto& frame : frames) {
          ...
          viewer->update_drawable("frame_coord_" + std::to_string(frame->id),
                                  glk::Primitives::coordinate_system(),
                                  guik::VertexColor(
                                    resolve_pose(frame) *
                                    Eigen::UniformScaling<float>(0.5f)));
          auto drawable =
            viewer->find_drawable("frame_" + std::to_string(frame->id));
          if (drawable.first) {
      ...
      invoke([this, keyframes] {
        ...
        for (const auto& keyframe : keyframes) {
          ...
          if (drawable.first == nullptr) {
            auto cloud_buffer =
              std::make_shared<glk::PointCloudBuffer>(keyframe->frame->points,
                                                      keyframe->frame->size());
    ...
    [this](gtsam_points::IncrementalFixedLagSmootherExtWithFallback& smoother,
      ...
      for (const auto& factor : new_factors) {
        ...
        if (factor->keys().size() == 1) {
          ...
          const auto matching_factor =
            dynamic_cast<gtsam_points::IntegratedMatchingCostFactor*>(
              factor.get());
          if (matching_factor) {
            const auto l = [this, idx0](const gtsam::NonlinearFactor* factor)
              ...
              const Eigen::Vector3d pt1 =
                static_cast<const gtsam_points::IntegratedMatchingCostFactor*>(
                  factor)
          ...
          if (gpu_factor) {
            const auto l = [this, idx0](const gtsam::NonlinearFactor* factor)
              ...
              const Eigen::Vector3f pt1 =
                static_cast<const gtsam_points::IntegratedVGICPFactorGPU*>(
                  factor)
                  ->get_fixed_target_pose()
                  .translation();
              return std::make_tuple(
          ...
          const auto l =
            [this, idx0, idx1](
              const gtsam::NonlinearFactor*) -> std::optional<FactorLine> {
            const auto found0 = odometry_poses.find(idx0);
      ...
      invoke([this, new_factor_lines] {
        ...
        viewer->update_drawable("odometry_factors",
                                std::make_shared<glk::ThinLines>(line_vertices,
                                                                 line_colors),
                                guik::VertexColor().set_alpha(factors_alpha));
      });
    });
    ...
    [this](int id, const EstimationFrame::ConstPtr& keyframe) {
      ...
      invoke([this, id, keyframe, frame] {
        ...
        if (id == 0) {
          ...
          if (drawable.first) {
            const Eigen::Isometry3f T_world_key0(
              drawable.first->cast<Eigen::Matrix4f>("T_world_key0"));
            const Eigen::Isometry3f T_center_key0(
              drawable.first->cast<Eigen::Matrix4f>("T_center_key0"));
        ...
        sub_viewer->update_drawable("frame_" + std::to_string(id),
                                    cloud_buffer,
                                    shader_setting);
        sub_viewer->update_drawable("coord_" + std::to_string(id),
                                    glk::Primitives::coordinate_system(),
                                    guik::VertexColor(
                                      shader_setting.cast<Eigen::Matrix4f>(
                                        "model_matrix") *
  ...
  SubMappingCallbacks::on_optimize_submap.add(
    [this](gtsam::NonlinearFactorGraph& graph, gtsam::Values& values) {
      ...
      invoke([this, factors] {
        ...
        sub_viewer->update_drawable("factors",
                                    std::make_shared<glk::ThinLines>(lines),
                                    guik::FlatGreen().set_alpha(factors_alpha));
      });
    });

  // Submap optimization status callback
  SubMappingCallbacks::on_optimization_status.add(
    [this](const gtsam_points::LevenbergMarquardtOptimizationStatus& status,
  ...
  GlobalMappingCallbacks::on_insert_submap.add(
    [this](const SubMap::ConstPtr& submap) {
      ...
      // To safely pass it to the UI thread (pass-by-value of Eigen classes can
      // crash) wrap it in a shared_ptr
      ...
      invoke([this, submap, T_world_origin] {
        ...
        auto viewer = guik::LightViewer::instance();
        auto cloud_buffer =
          std::make_shared<glk::PointCloudBuffer>(submap->frame->points,
                                                  submap->frame->size());
        auto shader_setting =
  ...
  GlobalMappingCallbacks::on_update_submaps.add(
    [this](const std::vector<SubMap::Ptr>& submaps) {
      ...
      invoke([this, latest_submap, submap_ids, submap_poses, mem_stats] {
        ...
        submap_memstats.insert(submap_memstats.end(),
                               mem_stats.begin(),
                               mem_stats.end());
        ...
        for (int i = 0; i < submap_ids.size(); i++) {
          ...
          auto_z_range[0] =
            std::min<float>(auto_z_range[0], submap_poses[i].translation().z());
          auto_z_range[1] =
            std::max<float>(auto_z_range[1], submap_poses[i].translation().z());

          auto drawable =
            viewer->find_drawable("submap_" + std::to_string(submap_ids[i]));
          if (drawable.first) {
        ...
        viewer->update_drawable(
          "factors",
          std::make_shared<glk::ThinLines>(submap_positions, indices),
          guik::FlatGreen().set_alpha(factors_alpha));
  ...
  GlobalMappingCallbacks::on_smoother_update_result.add(
    [this](gtsam_points::ISAM2Ext&             isam2,
      ...
      if (show_memory_stats) {
        ...
        for (int i = global_factor_stats_count;
             i < isam2.getFactorsUnsafe().size();
             i++) {
        ...
        invoke([this, mem_stats] {
          global_factor_memstats.insert(global_factor_memstats.end(),
                                        mem_stats.begin(),
                                        mem_stats.end());
        });
      }
...
void StandardViewer::viewer_loop() {
  ...
  viewer->register_drawable_filter("selection",
                                   [this](const std::string& name) {
                                     return drawable_filter(name);
                                   });
...
void StandardViewer::drawable_selection() {
  ...
  std::vector<const char*> current_color_modes = {"FLAT",
                                                  "INTENSITY",
                                                  "NORMAL"};
  ImGui::SetNextItemWidth(92);
  ImGui::Combo("color_mode",
               &current_color_mode,
               current_color_modes.data(),
  ...
  update_z_range |= ImGui::Combo("z_range_mode",
                                 &z_range_mode,
                                 z_range_modes.data(),
                                 z_range_modes.size());
  update_z_range |= ImGui::DragFloatRange2(
    "z_min", &z_range[0], &z_range[1], 0.1f, -10000.0f, 10000.0f);
  if (update_z_range) {
    Eigen::Vector2f z = z_range;
    if (z_range_mode == 0) {
      z += auto_z_range;
  ...
  if (ImGui::Checkbox("Cumulative rendering", &enable_partial_rendering)) {
    ...
    for (int i = 0;; i++) {
      ...
      auto cb =
        std::dynamic_pointer_cast<const glk::PointCloudBuffer>(found.second);
      auto cloud_buffer =
        std::const_pointer_cast<glk::PointCloudBuffer>(cb);  // !!
  ...
  if (show_odometry_status) {
    ImGui::Begin("odometry status",
                 &show_odometry_status,
    ...
    ImGui::Text("stamp:%.3f ~ %.3f",
                last_point_stamps.first,
                last_point_stamps.second);
    ImGui::Text("vel:%.3f %.3f %.3f",
                last_imu_vel[0],
                last_imu_vel[1],
                last_imu_vel[2]);
    ImGui::Text("bias:%.3f %.3f %.3f %.3f %.3f %.3f",
                last_imu_bias[0],
                last_imu_bias[1],
                last_imu_bias[2],
                last_imu_bias[3],
  ...
  if (show_memory_stats) {
    ...
    const size_t total_gpu =
      points_gpu + voxelmap_gpu + odom_gpu + factors_gpu + total_gl_bytes;
    const double total_cpu_mb = total_cpu * mb;
    const double total_gpu_mb = total_gpu * mb;

    ImGui::Text("Global mapping memory usage");
    if (ImGui::BeginTable("Global mapping memory usage",
                          3,
