...
size_t get_mem_stats_cpu(const PreprocessedFrame& frame) {
  ...
  if (frame.raw_points) {
    ...
    bytes +=
      sizeof(double) * frame.raw_points->size();  // raw points intensities
    bytes +=
      sizeof(Eigen::Vector4d) * frame.raw_points->size();  // raw points points
    bytes +=
      sizeof(Eigen::Vector4d) * frame.raw_points->size();  // raw points colors
...
size_t get_mem_stats_gpu(const gtsam_points::PointCloud& points) {
  ...
  bytes +=
    points.has_points_gpu() ? sizeof(Eigen::Vector3f) * points.size() : 0;
  bytes +=
    points.has_normals_gpu() ? sizeof(Eigen::Vector3f) * points.size() : 0;
...
size_t get_mem_stats_cpu(const gtsam_points::GaussianVoxelMap* voxelmap) {
  const auto v =
    dynamic_cast<const gtsam_points::GaussianVoxelMapCPU*>(voxelmap);
  if (!v) {
    return 0;
  }

  return v->num_voxels() * (sizeof(gtsam_points::VoxelInfo) +
                            sizeof(gtsam_points::GaussianVoxel));
}

size_t get_mem_stats_gpu(const gtsam_points::GaussianVoxelMap* voxelmap) {
#ifdef GTSAM_POINTS_USE_CUDA
  const auto v =
    dynamic_cast<const gtsam_points::GaussianVoxelMapGPU*>(voxelmap);
  if (!v) {
    return 0;
  }
  constexpr size_t voxel_size =
    sizeof(int) + sizeof(Eigen::Vector3f) + sizeof(Eigen::Matrix3f);
  return v->voxelmap_info.num_voxels * voxel_size +
         v->voxelmap_info.num_buckets * sizeof(gtsam_points::VoxelBucket);
#else
...
size_t get_mem_stats_cpu(const EstimationFrame& frame) {
  ...
  bytes +=
    sizeof(Eigen::Isometry3d) * 3;   // T_lidar_imu, T_world_lidar, T_world_imu
  bytes += sizeof(Eigen::Vector3d);  // v_world_imu
  bytes += sizeof(Eigen::Matrix<double, 6, 1>);  // imu_bias
  bytes += frame.imu_rate_trajectory.size() * sizeof(double) *
           8;                // IMU-rate trajectory
...
SubMapMemoryStats::SubMapMemoryStats(const SubMap& submap)
  : SubMapMemoryStats() {
  id              = submap.id;
...
FactorMemoryStats::FactorMemoryStats(
  ...
  : FactorMemoryStats() {
  auto mcf =
    dynamic_cast<gtsam_points::IntegratedMatchingCostFactor*>(factor.get());
  if (mcf) {
